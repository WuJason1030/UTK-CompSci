/* Searching */
/* We are going to cover two searching algorithms. By searching, I mean seeing if an element exists
...
/* Linear search */
/* Linear search sequentially checks each element of the vector/array until a match is found or 
   the whole vector/array has been searched.
   In the worst case, we would have to check all 'n' elements of the vector/array, where 'n' is the
   size of the vector/array.
   Linear search is honestly just a fancy way of saying "for loop".


    In the code snippet below, we are holding data in the array, passing the size of the array as
...
/* Binary search */
/* Binary search is a faster way of finding an element in a list-like data structure,
   BUT the array/vector must be sorted first.
   With binary search, we search a sorted array by repeatedly dividing the search interval in half.
   ...
   I know I can eliminate the bottom half of this sub-sub-array, which only leaves me with one element left.

   I still need to check that element (what if it's 7?) just to make sure the element
   exists in the array at all.
   It is 6, so we found it!

   Notice this was the worst case scenario, but we only did 4 checks for an array of size 10.
   In the worst case scenario of linear search, we would do 10 checks.
   HOWEVER, we must know the array is sorted! And that can take time too if it's not already.
   ...
   Submit for today's participation grade - you must have recursive solution attempt to receive credit.
